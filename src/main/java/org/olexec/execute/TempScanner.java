 ...
 * TempScanner(InputStream source)
 *
 * 然后在把所有报错的地方修改一下，主要修改：
 * 1. 构造函数的类名：Scanner -> TempScanner
 * 2. 几个方法的返回参数类型：java.util.Scanner -> org.olexec.execute.TempScanner
 *
 * PS. Scanner 类要是能 extends 就好了，这样就不用写这么多了，现在这个类看起来好吓人...
 */
public final class TempScanner implements Iterator<String>, Closeable {
     ...
     */
    private TempScanner(Readable source, Pattern pattern) {
     ...
     *         interface
     */
    public TempScanner(Readable source) {
     ...
     */
    public TempScanner(InputStream source) {
        // 判断一下输入的 InputStream 是不是 TempInputStream，
        // 如果是就调用一下它的 get 方法，把当前线程的输入流从 ThreadLocal 中取出来传入
        this(new InputStreamReader(
                (source instanceof TempInputStream) ? ((TempInputStream) source).get() : source),
                WHITESPACE_PATTERN);
    }
     ...
     *         does not exist
     */
    public TempScanner(InputStream source, String charsetName) {
     ...
     */
    public TempScanner(File source) throws FileNotFoundException {
     ...
     *         not found
     */
    public TempScanner(File source, String charsetName)
            throws FileNotFoundException
    {
        this(Objects.requireNonNull(source), toDecoder(charsetName));
    }

    private TempScanner(File source, CharsetDecoder dec)
            throws FileNotFoundException
    {
     ...
     * @since   1.7
     */
    public TempScanner(Path source)
            throws IOException
    {
     ...
     * @since   1.7
     */
    public TempScanner(Path source, String charsetName) throws IOException {
        this(Objects.requireNonNull(source), toCharset(charsetName));
    }

    private TempScanner(Path source, Charset charset)  throws IOException {
     ...
     */
    public TempScanner(String source) {
     ...
     */
    public TempScanner(ReadableByteChannel source) {
     ...
     *         does not exist
     */
    public TempScanner(ReadableByteChannel source, String charsetName) {
     ...
     * @return this scanner
     */
    public TempScanner useDelimiter(Pattern pattern) {
        delimPattern = pattern;
        return this;
    }
     ...
     * @return this scanner
     */
    public TempScanner useDelimiter(String pattern) {
     ...
     * @return this scanner
     */
    public TempScanner useLocale(Locale locale) {
     ...
     */
    public TempScanner useRadix(int radix) {
     ...
     * @throws IllegalStateException if this scanner is closed
     */
    public TempScanner skip(Pattern pattern) {
        ensureOpen();
        if (pattern == null)
            throw new NullPointerException();
        clearCaches();
     ...
     * @throws IllegalStateException if this scanner is closed
     */
    public TempScanner skip(String pattern) {
